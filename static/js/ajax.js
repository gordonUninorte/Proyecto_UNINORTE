function cargarMensaje()
{
    ...
    objXMLHTTP.open('GET', 'http://127.0.0.1:5000/login');
...
function guardarMensaje()
{
    ...
    objXMLHTTP.open('POST', 'http://127.0.0.1:5000/login');

    objXMLHTTP.addEventListener('load', agregarCompletado);
    
    objXMLHTTP.setRequestHeader("Content-Type", "application/json")

    objXMLHTTP.send(JSON.stringify({'usuario':'Pepe', 'asunto':'prueba', 'mensaje':'Hola mundo'}));
}

function agregarCompletado(evt)
{
    var data =  JSON.parse(this.response);
